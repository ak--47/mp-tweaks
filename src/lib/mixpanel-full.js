...
(function () {
	...
	var unencodedJS = `(() => {
		...
		function encode(buffer) {
		  ...
		  for (let i = 0; i < bytes.length; i += 3) {
			b64 += chars[bytes[i] >> 2];
			b64 += chars[((bytes[i] & 3) << 4) | (bytes[i + 1] >> 4)];
			b64 += chars[((bytes[i + 1] & 15) << 2) | (bytes[i + 2] >> 6)];
			b64 += chars[bytes[i + 2] & 63];
		  }
		  if (bytes.length % 3 === 2) {
			b64 = b64.slice(0, -1) + "=";
		...
		async function getTransparentBlobFor(width, height, opts) {
		  ...
		  if ("OffscreenCanvas" in globalThis) {
			...
			off.getContext("2d");
			const blob = await off.convertToBlob(opts);
			const buf = await blob.arrayBuffer();
			const b64 = encode(buf);
			transparentBlobMap.set(id, b64);
			return b64;
		  }
		  return "";
		}
		...
		self.onmessage = async (e) => {
		  ...
		  if ("OffscreenCanvas" in globalThis) {
			const transparentP = getTransparentBlobFor(width, height, dataURLOptions);
			const off = new OffscreenCanvas(width, height);
			const ctx = off.getContext("2d");
			ctx.drawImage(bitmap, 0, 0);
			bitmap.close();
	  
			const blob = await off.convertToBlob(dataURLOptions);
			const buf = await blob.arrayBuffer();
			const b64 = encode(buf);
			const transparentB64 = await transparentP;
	  
			if (!lastBlobMap.has(id) && transparentB64 === b64) {
			  lastBlobMap.set(id, b64);
			  return self.postMessage({ id });
			}
			if (lastBlobMap.get(id) === b64) {
			  return self.postMessage({ id });
			}
    ...
    function WorkerWrapper(options) {
        ...
        try {
		    ...
		    const base64 = btoa(unencodedJS);
    		return new Worker(
      		`data:text/javascript;base64,${base64}`,{ name: options.name });
        } finally{
